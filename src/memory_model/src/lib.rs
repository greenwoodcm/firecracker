...
static USE_HUGE_PAGES: AtomicBool = AtomicBool::new(false);

/// Dummmy comment.
pub fn set_use_huge_pages(value: bool) {
    USE_HUGE_PAGES.store(value, Ordering::Relaxed);
}

/// Dummmy comment.
pub fn use_huge_pages() -> bool {
    USE_HUGE_PAGES.load(Ordering::Relaxed)
}
